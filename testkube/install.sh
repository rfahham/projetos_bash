...
_download_url() {
  ...
  if [ -z "$TESTKUBE_VERSION" ]; then
    if [ "$1" = "beta" ]; then
      tag="$(curl -s "https://api.github.com/repos/kubeshop/testkube/releases" 2>/dev/null | jq -r '.[].tag_name | select(test("beta"))' | head -n 1 )"
        if [ -z "$tag" ]; then
            echo "No beta releases found. Installing latest release" >&2
            tag="$(
              curl -s "https://api.github.com/repos/kubeshop/testkube/releases/latest" 2>/dev/null | jq -r '.tag_name')"
        fi
    else
      tag="$(curl -s "https://api.github.com/repos/kubeshop/testkube/releases/latest" 2>/dev/null | jq -r '.tag_name')"
    fi
  else
